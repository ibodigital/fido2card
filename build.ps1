...
Invoke-PS2EXE `
    ...
    -OutputFile (Join-Path $scriptDir "fido2lock-service.exe") `
    -NoConsole `
    -RequireAdmin `
...
Invoke-PS2EXE `
    ...
    -OutputFile (Join-Path $scriptDir "fido2lock-tray.exe") `
    -NoConsole `
...
Write-Host "Output:"
Write-Host "  $scriptDir\fido2lock-service.exe"
Write-Host "  $scriptDir\fido2lock-tray.exe"
Write-Host ""
Write-Host "Next: copy this folder to the target machine and run deploy.ps1"
